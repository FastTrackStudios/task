...
- `apps/watchos/TaskWatch/Model/PhoneSync.swift` — a `WCSessionDelegate`
...
2. **Sender (iOS shell)** — `apps/mobile/src/watch_sync.rs`.
   ...
   `apps/mobile/Cargo.toml`); other targets get a no-op `init()`.
