...
fn host(tmp: &std::path::Path) -> (LiveHost, String) {
    ...
    let set = collections
        .create("live-test".into(), "Worship Set".into(), CollectionKind::new("songlist"))
        .unwrap();
    for slug in ["washed", "who-else"] {
        collections
            .add_item(Placement { collection_id: set.id.clone(), node: NodeRef::song(slug), after: None })
            .unwrap();
    }
    let resources = resources::ResourcesBackend::new(tmp.join("resources"));
    (LiveHost::new("live-test".into(), collections, resources), set.id)
}
...
fn member(host: &LiveHost) -> LocalServer {
    ...
    let router = LayerRouter::new()
        .with(live_proto::live_sessions_rpc_service_descriptor(), live_proto::serve(LiveLane(host.clone())))
        .merge(live_proto::stream_layer(LiveLane(host.clone())))
        .with(crdt::sync::doc_sync_service_descriptor(), crdt::sync::DocSyncDispatcher::new(only));
...
fn tempo(doc: &CrdtDoc) -> Option<String> {
    doc.loro().get_map("song").get("tempo").and_then(|v| v.into_value().ok()).and_then(|v| v.into_string().ok()).map(|s| s.to_string())
}
...
async fn two_peers_meet_in_a_songs_doc_through_task() {
    ...
    first.loro().commit();
    eventually("the second peer has the first's edit", || tempo(&second).as_deref() == Some("78")).await;
}
...
async fn a_doc_nobody_joined_is_not_served() {
    let tmp = tempfile::tempdir().unwrap();
    let (host, setlist) = host(tmp.path());
    assert!(!host.admits(task_server::live::doc_id("live-test", &setlist, 0, "washed")), "not before a join");
    host.join(&setlist, None).unwrap();
    assert!(host.admits(task_server::live::doc_id("live-test", &setlist, 0, "washed")));
...
async fn a_playgrounds_epoch_moves_on_and_every_peer_is_told() {
    ...
    eventually("the subscription attached", || host.epochs_hub().subscriber_count() > 0).await;
    ...
    assert!(!host.admits(old), "the old epoch's docs are no longer served");

    let again = live.join(setlist.clone()).await.unwrap();
    assert_eq!(again.epoch, 1);
    assert_ne!(again.songs[0].doc_id, set.songs[0].doc_id, "the song starts over under a new doc");
}
...
async fn a_guest_is_kept_to_its_links_set_and_given_its_songs_files() {
    let tmp = tempfile::tempdir().unwrap();
    let (host, setlist) = host(tmp.path());
    let files = std::collections::HashMap::from([("washed".to_owned(), "https://task.test/org/x/share/w".to_owned())]);
    ...
    let router = LayerRouter::new().with(live_proto::live_sessions_rpc_service_descriptor(), live_proto::serve(lane));
    ...
    assert_eq!(live.join(String::new()).await.unwrap().setlist, setlist, "empty is the link's set");
}
