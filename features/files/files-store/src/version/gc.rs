...
pub struct GcStats {
    ...
    /// `crate::chunk::ChunkStore::gc`): manifests removed now,
    /// their now-unreferenced chunks left to iroh-blobs' own background
    /// sweep to reclaim.
    pub chunks: crate::chunk::GcStats,
}

fn chunk_file_id(id: &jj_lib::backend::FileId) -> Option<crate::chunk::FileId> {
    crate::chunk::FileId::from_hex(&id.hex()).ok()
}
...
async fn mark_tree(
    ...
    live_files: &mut BTreeSet<crate::chunk::FileId>,
) -> Result<()> {
...
) -> Result<(usize, BTreeSet<crate::chunk::FileId>)> {
    let heads: Vec<CommitId> = index
        .all_heads_for_gc()
        .map_err(|e| crate::version::error::Error::Repo(e.to_string()))?
        .collect();

    let mut live_commits = BTreeSet::new();
    let mut live_trees = BTreeSet::new();
    let mut live_copies = BTreeSet::new();
    let mut live_files: BTreeSet<crate::chunk::FileId> = BTreeSet::new();
