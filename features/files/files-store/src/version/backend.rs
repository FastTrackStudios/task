...
//! [`crate::chunk::ChunkStore`] (FastCDC/blake3/iroh-blobs);
...
use crate::version::objects::ObjectStore;
...
fn chunk_file_id(id: &FileId) -> Result<crate::chunk::FileId> {
    crate::chunk::FileId::from_hex(&id.hex()).map_err(Error::from)
}

fn jj_file_id(id: crate::chunk::FileId) -> FileId {
...
pub struct VersionStoreBackend {
    chunks: Arc<crate::chunk::ChunkStore>,
    objects: ObjectStore,
...
impl std::fmt::Debug for VersionStoreBackend {
    // `crate::chunk::ChunkStore` doesn't derive `Debug`; the
...
/// Re-exports `crate::chunk::gc::DEFAULT_INTERVAL` rather than
/// hardcoding its own copy, so the two layers' production cadence can't
/// silently diverge.
pub const DEFAULT_GC_INTERVAL: Duration = crate::chunk::gc::DEFAULT_INTERVAL;
...
impl VersionStoreBackend {
    ...
    pub async fn open_with_gc_interval(
        ...
        let chunks = crate::chunk::ChunkStore::open_with_gc(
            root.join("chunks"),
            crate::chunk::ChunkerConfig::default(),
            crate::chunk::GcConfig {
    ...
    pub fn chunks(&self) -> &Arc<crate::chunk::ChunkStore> {
        &self.chunks
    }
...
impl Backend for VersionStoreBackend {
    ...
    fn get_copy_records(
        ...
        let head = head.clone();
        let records = self
            .block_on(crate::version::chain::copy_records_between(
                self,
                paths.as_deref(),
                &root,
                &head,
