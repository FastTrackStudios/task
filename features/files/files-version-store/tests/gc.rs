...
use task_files_version_store::gc::sweep;
use task_files_version_store::repo::init_repo_with_gc_interval;
...
fn chunk_id_of(jj_id: &jj_lib::backend::FileId) -> task_files_chunk_store::FileId {
    task_files_chunk_store::FileId::from_hex(&jj_id.hex()).unwrap()
}
...
async fn wait_until_chunk_absent(
    ...
    id: task_files_chunk_store::FileId,
) {
