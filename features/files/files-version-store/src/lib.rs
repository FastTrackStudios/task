...
//! chunk store ([`task_files_chunk_store`]), per ADR 0001
...
//!
//! ```no_run
//! # async fn example() -> task_files_version_store::Result<()> {
//! use task_files_version_store::checkpoint::{checkpoint, Change};
//! use task_files_version_store::{chain, repo};
...
//! )
//! .await?;
//! let backend = repo2.store().backend_impl::<task_files_version_store::VersionStoreBackend>().unwrap();
