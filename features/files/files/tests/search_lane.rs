...
async fn extracted_text_makes_a_file_findable() {
    ...
    assert!(
        backend.search(query("dog", root_id)).await.unwrap().is_empty(),
...
async fn a_hit_addresses_a_region_and_not_the_whole_file() {
    ...
    let Region::Bytes { start, end } = hits[0].region else {
        panic!("a text hit is a byte range, not `Whole`: {:?}", hits[0].region);
    };
...
async fn the_sidecar_is_an_ordinary_readable_file_beside_the_content() {
    ...
    assert!(text.contains("@ 0 42"), "byte offsets are in the file: {text}");
    ...
    assert_eq!(
        ...
        text.replace(
            text.lines().find(|l| l.starts_with("extracted-at:")).unwrap(),
    ...
    assert_eq!(backend.search(query("dog", root_id)).await.unwrap().len(), 1);
}
...
async fn re_extracting_unchanged_content_is_a_no_op() {
    ...
    assert!(third[0].updated_at > first[0].updated_at, "changed content is redone");
    assert_eq!(backend.search(query("heron", root_id)).await.unwrap().len(), 1);
    assert!(
        backend.search(query("dog", root_id)).await.unwrap().is_empty(),
...
async fn a_file_that_fails_extraction_does_not_block_browsing_it() {
    ...
    assert!(!failed.done);
    assert!(
        failed.failed.as_deref().is_some_and(|r| r.contains("not UTF-8")),
        "the reason is named: {:?}",
        failed.failed
    );
    assert!(
...
async fn speech_is_refused_by_name() {
    ...
    assert!(reason.starts_with("not yet implemented: speech transcription"), "{reason}");
}
...
async fn vision_is_refused_by_name() {
    ...
    assert!(reason.starts_with("not yet implemented: visual description"), "{reason}");
}
...
async fn pending_names_what_is_not_searchable_yet() {
    ...
    assert!(
        before.iter().any(|s| s.path == note() && s.kind == Extract::Text),
...
async fn a_derived_index_is_not_extracted_in_turn() {
    ...
    let again = backend
        ...
        .expect("and again");
    assert!(
        !again.iter().any(|s| s.path.as_str().ends_with(".extract.txt")),
