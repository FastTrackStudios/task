...
//! Verbatim except visibility (`pub(crate)` → `pub(super)`)
//! so the wrapper crate can reach into the `vendor` module.
...
pub(super) fn tokio_command(program: impl AsRef<OsStr>) -> Command {
...
#[allow(dead_code)]
pub(super) fn std_command(program: impl AsRef<OsStr>) -> std::process::Command {
    let mut command = std::process::Command::new(program);
    hide_console_on_windows(&mut command);
    command
}

pub(super) async fn kill_child_process_tree(child: &mut Child) {
    #[cfg(windows)]
    {
...
#[cfg(target_os = "windows")]
pub(super) fn resolve_windows_executable(program: &str, path_env: Option<&str>) -> Option<PathBuf> {
...
#[cfg(target_os = "windows")]
pub(super) fn build_cmd_c_command(program: &Path, args: &[String]) -> Result<String, String> {
